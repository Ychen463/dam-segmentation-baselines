"""Step-4 SegFormer-B2 config: two presets (Run A = plain, Run B = static curriculum).

Inherits data protocol (splits, class names, loss weights, device, seed) from
baseline_unet.config so that all steps share a byte-identical data pipeline.
"""
from __future__ import annotations

import os
from dataclasses import dataclass
from pathlib import Path

from baseline_unet import config as base

# ----- Shared (inherited) -----
DATA_ROOT = base.DATA_ROOT
NUM_CLASSES = base.NUM_CLASSES
CLASS_NAMES = base.CLASS_NAMES
SPLITS_DIR = base.SPLITS_DIR
SPLIT_FILES = {k: base.SPLITS_DIR / f"{k}.txt" for k in ("train", "val", "test")}

PKG_DIR = Path(__file__).resolve().parent
RUNS_DIR = PKG_DIR / "runs"
RUNS_DIR.mkdir(parents=True, exist_ok=True)

# ----- Loss / training defaults shared across presets -----
LOSS_CE_W = base.LOSS_CE_W
LOSS_DICE_W = base.LOSS_DICE_W
CE_WEIGHTS = base.CE_WEIGHTS  # manual fallback
SEED = base.SEED
DEVICE = base.DEVICE

# Pretrained HuggingFace model ID (change to local cache path if needed).
PRETRAINED_MODEL_ID = "nvidia/segformer-b2-finetuned-ade-512-512"

# Boundary F1 absolute pixel tolerance.
BF1_TOLERANCE_PX = 2

# MPS op fallback.
os.environ.setdefault("PYTORCH_ENABLE_MPS_FALLBACK", "1")


@dataclass
class RunCfg:
    name: str
    pretrained: str          # HF model id
    img_size: int
    batch_size: int
    grad_accum: int
    epochs: int
    lr: float
    weight_decay: float = 1e-4
    warmup_epochs: int = 5
    curriculum: str = "none"  # "none" | "static"


# Preset A: plain baseline
RUN_A = RunCfg(
    name="segformer_b2_plain_512",
    pretrained=PRETRAINED_MODEL_ID,
    img_size=512, batch_size=4, grad_accum=1,
    epochs=100, lr=6e-5, warmup_epochs=5,
    curriculum="none",
)

# Preset B: official static curriculum
RUN_B = RunCfg(
    name="segformer_b2_staticcurr_512",
    pretrained=PRETRAINED_MODEL_ID,
    img_size=512, batch_size=4, grad_accum=1,
    epochs=100, lr=6e-5, warmup_epochs=5,
    curriculum="static",
)

PRESETS = {"A": RUN_A, "B": RUN_B}


def run_dir(cfg: RunCfg) -> Path:
    d = RUNS_DIR / cfg.name
    d.mkdir(parents=True, exist_ok=True)
    (d / "samples").mkdir(parents=True, exist_ok=True)
    return d
