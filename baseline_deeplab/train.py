"""Step-1 DeepLabV3+ training.

Two presets selectable with --preset {A,B}:
    A = deeplabv3p_r50_512  (practical baseline, 50 ep, r50, bs=4, lr=5e-4)
    B = deeplabv3p_r34_320  (head-only control vs Step 0, 30 ep, r34, bs=8, lr=1e-3)

Reuses baseline_unet's dataset, losses, splits and visualize modules verbatim.

Usage:
    python -m baseline_deeplab.train --preset B --dry-run
    python -m baseline_deeplab.train --preset B
    python -m baseline_deeplab.train --preset A --epochs 10              # probe
    python -m baseline_deeplab.train --preset A --resume runs/deeplabv3p_r50_512/last.pt
"""
...
def oom_probe(model: nn.Module, cfg: C.RunCfg, device: str) -> bool:
    """Return True if a 1-step (bs, 3, H, W) forward+backward succeeds."""
    try:
        x = torch.zeros(cfg.batch_size, 3, cfg.img_size, cfg.img_size, device=device)
        y = torch.zeros(cfg.batch_size, cfg.img_size, cfg.img_size, device=device,
                        dtype=torch.long)
        logits = model(x)
...
@torch.no_grad()
def evaluate(model, loader, criterion, device, metrics: SegMetricsBF1) -> Dict[str, float]:
    model.eval()
    metrics.reset()
    loss_sum = 0.0
    n_batches = 0
...
def main() -> None:
    ...
    parser.add_argument("--epochs", type=int, default=None,
                        help="override preset epochs (e.g. 10 for Run A probe)")
    ...
    try:
        ...
        smp_v = _smp.__version__
    except Exception:
        smp_v = "?"
    print(f"[train] preset={args.preset} ({cfg.name})")
    print(f"[train] device={device}  torch={_t.__version__}  smp={smp_v}")
    print(f"[train] cfg: encoder={cfg.encoder} img={cfg.img_size} bs={cfg.batch_size}"
    ...
    # ----- OOM probe (skip in dry-run; dry-run does its own 1 batch forward) -----
    accum_notice = None
    ...
    with open(report, "w") as f:
        ...
        f.write(f"encoder: {cfg.encoder}  img_size: {cfg.img_size}  "
    ...
    # Render curves from metrics.csv
    try:
